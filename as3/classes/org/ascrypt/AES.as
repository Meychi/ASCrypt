package org.ascrypt
{
	...
	*/
	public class AES
	{
		...
		*/
		private static const ERROR_KEY:String = "Invalid key size. Key size needs to be either 128, 192 or 256 bits.\n";
		private static const ERROR_MODE:String = "Invalid mode of operation. Supported modes are ECB, CBC, CTR or NONE.\n";
		private static const ERROR_BLOCK:String = "Invalid block size. Block size is fixed at 128 bits.\n";
		...
		*/
		public static function encrypt(key:Array, bytes:Array, mode:String = "ecb", iv:Array = null):Array
		{
			check(key, bytes);
		...
		*/
		public static function decrypt(key:Array, bytes:Array, mode:String = "ecb", iv:Array = null):Array
		{
			check(key, bytes);
			var k:Array = key.concat();
			var b:Array = bytes.concat();
		...
		*/
		private static function init():void
		{
			...
			isbox = new Array(256);
			xtime = new Array(256);
			for (var i:int = 0; i < 256; i++) isbox[sbox[i]] = i;
			for (var j:int = 0; j < 16; j++) isrtab[srtab[j]] = j;
		...
		{
			var kl:int = k.length;
			if (kl != 16 && kl != 24 && kl != 32) throw Error(ERROR_KEY);
			if (b.length % 16 != 0) throw Error(ERROR_BLOCK);
		}
